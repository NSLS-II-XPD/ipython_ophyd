...
def better_temp_gas_run(
        
        
        gasses, temp_list, time_list, calibration_uid,
                        background_uid,
                        is_background, dets=[pe1c, gas.current_gas, eurotherm],
                        md=None):
    """
    ...
    Optionally, preview the plan.
    >>> print_summary(better_temp_gas_run(["N", "Ar"]*5, [50]*10, list(range(0, 30*10, 30)), 'hello', 'world', True))
    ...
    assert len(temp_list) == len(time_list)
    ...
    # Here we heat/cool until at t=0 temperature
    ...
    def inner():
        ...
        for i, (gas_selection, temp, time_position) in enumerate(
                zip(gasses, temp_list, time_list)):
            # Change gasses
            ...
            if i + 1 < len(temp_list):
                yield from abs_set(euro_ramp_rate,
                                   np.abs(temp_list[i + 1] - eurotherm.get()) /
                                   time_list[i + 1], wait=False)
                yield from abs_set(eurotherm, temp_list[i + 1], wait=False)
                print('ramp_rate', euro_ramp_rate.get())
                yield from timed_count_guts(dets, total_time=time_list[i + 1],
                                            md=cal_bg_asoc)
